...
def calculadora(query: str) -> list:
    try:
        ...
        # Si no es ecuación, la tratamos como expr = 0
        if "=" in query:
        ...
        try:
            soluciones = solve(expr)
        except Exception:
            # fallback por si solve no puede resolver simbolicamente
            soluciones = nroots(expr)

        return [str(s) for s in soluciones]

    except SympifyError:
        raise ValueError("Expresión no válida")
    except Exception as e:
        raise ValueError(f"No se pudo resolver la ecuación: {str(e)}")
